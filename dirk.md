...
s is a value masked by the phase operator b, allowing you to encode relational structure (cross-correlate serial values).
...
```
ia(t)=b(t)
i*da/dt = db/dt => eignestate => d/di*i(da/dt) = 0 
i*∫₀ᴺ(da/dt)*dn = ∫₀ᴺ(db/dt)*dn
d/di*(da/dt) = 0 => (da/dt)/dn = 0 
i*(1/N)*da/dt = ∫₀ᴺ((db/dt)*dn
i => ∫₀ᴺ(da/dt)*dn = 1 -> normalized coordinates
∫₀ᴺ(db/dt)*dn = i -> transform or eigenvalue
(i/N)*(da/dt) = i(N) = Nth order expansion of 1
a(t)dt = i · ∫b(r)dr
∫₀ᴺ a(t)dt = i · ∫₀ᴺ b(r)dr
```
